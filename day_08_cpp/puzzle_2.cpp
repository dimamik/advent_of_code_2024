// g++ -std=c++11 -o puzzle_2 puzzle_2.cpp && ./puzzle_2
#include <iostream>
#include <fstream>
#include <vector>
#include <string>
...
int main()
{
    ...
    // 2b. Append all generated antinodes to RESULT -> O(N)
    // 3. Count the number of antinodes in RESULT array

    // Total complexity -> O(N^5) where N is max(rowsNumber, colsNumber)
    ...
    for (const char ch : supportedChars)
    {
        ...
        for (size_t i = 0; i < positions.size(); ++i)
        {
            for (size_t j = i + 1; j < positions.size(); ++j)
            {
                ...
                while (isInGrid(rowsNumber, colsNumber, row1, col1))
                {
                ...
                while (isInGrid(rowsNumber, colsNumber, row2, col2))
                {
